...
var frame = 0;
var sf = Math.min(canvas.width, canvas.height) / 1.5; //scale factor
var grd = ctx.createRadialGradient(canvas.width / 2, canvas.height / 2, 0, canvas.width / 2, canvas.height / 2, sf * 1.2);
...
for (var i = 0; i < num_flowers; i++) {
    fs.push({ size: rand(1, 10), petals: rand(4, 12), rot: rand(-5, 5), c: [rand(100, 255), rand(100, 255), rand(100, 255)], x: canvas.width / 2, y: canvas.height / 2 });
}
