...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
/* returns a pseudo-random number generator function.
  Two generators created with the same seed will return the same sequence of numbers
  The returned generator function returns numbers in the range 0..1
*/
function prng(seed) {
...
/* returns a function to generate noise for something circular
i.e. a continuous periodical function with period period.
The value returned is between min and max.
harmonics is an array of{frq, ampl} objects with frq integer >=2 (number of random points by period)
and ampl a relative amplitude. ampl should be positive, their sum non-zero.
A pseudo random numbers generator, which generates values between 0 and 1,
may be provided, by example for reproducible noise
remark 1 : a random phase shift is used for every harmonic - no check for frq value,
may lead to non continuous function if not integer values.

The average value returned is NOT guaranteed to be 0, and won't be
*/
...
/* creates an ellipse with horizontal axis 1, vertical axis kab 
 creates nbPoints around it
 The center is located at (0, 0)
 The points are moved from their original place to give a nice flower shape,
 internally tangent to the "true" ellipse
*/
...
/* we will slightly move these points by adding a noise to the radius
To make a nice curve between the center and the point, we will create two slightly
modified copies of the list of angles : one for the start of the curve at the center,
the other for the end at the point
*/
/* noise on radius */
...
/* spreads points as regularly as possible around the ellipse
returns array of {r, angle} with 1st point having r = 1 and angle 0
*/
...
//  drawPoints(points, 5,"#f00");
/* we set points to the places given by the angles
 then we compute the actual distances between points, and their average distance
 then we change the angles to compensate for the ratio between actual and average distance
We iterate this process until the min distance / max distance ratio is "very" close to 1,
which means distances are almost the same.
*/
  while (true) {
...
// calculates xc and yc of flower too (it's the end of the stem)
  ...
  const p1y = maxy - alpha * this.hStem / 2;
  const p2x = this.xStem + alpha * this.dxStem;
  const p2y = p1y - this.hStem / 2

  this.yc = (1 - alpha) * p1y + alpha * p2y;
  this.xc = (1 - alpha) * this.xStem + alpha * p2x;

  ctx.moveTo (this.xc, this.yc);
  ctx.quadraticCurveTo(this.xStem, p1y, this.xStem, this.yStem);
  this.pathStem = ctx;
